...
Copy-paste these into App Store Connect → App Store → 1.0 Prepare for Submission. Replace placeholders in `<>` with your actual values before pasting.
...
4+ — no objectionable content. The app contains audio playback only; bundled audio is public-domain literature read aloud.

## Content Rights

Yes — contains third-party content (LibriVox public-domain recordings, credited in the privacy policy and onboarding).
